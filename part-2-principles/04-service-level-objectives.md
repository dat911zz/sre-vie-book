...
-   *Các hệ thống phục vụ hướng người dùng* (user-facing serving systems), như các frontend tìm kiếm Shakespeare, nhìn chung quan tâm đến *availability* (khả dụng), *latency* (độ trễ), và *throughput* (thông lượng). Nói cách khác: Chúng tôi có thể phản hồi yêu cầu không? Mất bao lâu để phản hồi? Bao nhiêu yêu cầu có thể được xử lý?
