...
Hệ thống thiếu tin cậy có thể nhanh chóng làm xói mòn niềm tin của người dùng, nên chúng tôi muốn giảm thiểu khả năng hệ thống gặp sự cố. Tuy nhiên, kinh nghiệm cho thấy khi xây dựng hệ thống, chi phí không tăng tuyến tính theo mức độ tin cậy — một bước cải thiện độ tin cậy tiếp theo có thể tốn kém gấp 100 lần so với bước trước đó. Sự tốn kém này thể hiện ở hai khía cạnh:
...
Áp dụng công thức này cho khoảng thời gian một năm, chúng ta có thể tính được số phút downtime cho phép để đạt mức nine khả dụng nhất định. Chẳng hạn, hệ thống đặt mục tiêu khả dụng 99.99% được phép down tối đa 52.56 phút trong năm mà vẫn nằm trong ngưỡng khả dụng; xem [Availability Table](https://sre.google/sre-book/availability-table#appendix_table-of-nines) để tham khảo bảng chi tiết.

Tuy nhiên, tại Google, các metrics khả dụng theo thời gian thường không có ý nghĩa vì chúng tôi vận hành trên các dịch vụ phân tán toàn cầu. Cách tiếp cận cách ly lỗi (fault isolation) của chúng tôi đảm bảo gần như chắc chắn rằng vào bất kỳ lúc nào, chúng tôi cũng đang phục vụ ít nhất một phần traffic của một dịch vụ ở đâu đó trên thế giới (tức là luôn "up" (hoạt động) ít nhất một phần). Vì vậy, thay vì dùng các metrics quanh uptime, chúng tôi định nghĩa khả dụng theo *request success rate* (tỷ lệ yêu cầu thành công). [Khả dụng tổng hợp](#kha-dung-tong-hop) cho thấy cách metrics dựa trên yield này được tính trên một cửa sổ trượt (tức là tỷ lệ các yêu cầu thành công trong một ngày).
...
Ví dụ, một hệ thống xử lý 2.5 triệu yêu cầu mỗi ngày với mục tiêu khả dụng hàng ngày 99.99% có thể phát sinh tối đa 250 lỗi mà vẫn đạt chỉ tiêu trong ngày đó.
...
Việc lượng hóa downtime không lên kế hoạch thông qua request success rate cũng giúp các metrics khả dụng này dễ sử dụng hơn với những hệ thống không trực tiếp phục vụ người dùng cuối. Phần lớn các hệ thống không phục vụ (ví dụ batch, pipeline, storage, transactional) đều có khái niệm rõ ràng về đơn vị công việc thành công và thất bại. Đúng là các hệ thống được bàn trong chương này chủ yếu là hệ thống phục vụ người tiêu dùng và cơ sở hạ tầng, nhưng nhiều nguyên lý tương tự cũng áp dụng được cho các hệ thống không phục vụ với rất ít điều chỉnh.

Ví dụ, một quá trình batch trích xuất, chuyển đổi và chèn nội dung từ một trong các database của khách hàng vào data warehouse (kho dữ liệu) để phân tích thêm, có thể được đặt chạy định kỳ. Dựa trên request success rate được định nghĩa theo các bản ghi xử lý thành công và thất bại, chúng tôi vẫn tính được một metrics khả dụng hữu ích, dù hệ thống batch không chạy liên tục.

Cách làm phổ biến nhất là đặt mục tiêu khả dụng hàng quý cho từng dịch vụ, sau đó theo dõi hiệu suất so với mục tiêu đó theo tuần, thậm chí theo ngày. Chiến lược này giúp chúng tôi quản lý dịch vụ dựa trên một mục tiêu khả dụng tổng quát bằng cách tìm, truy tìm và sửa các sai lệch có ý nghĩa khi chúng không thể tránh khỏi phát sinh. Xem [Service Level Objectives](04-service-level-objectives.md) để biết thêm.

## Mức độ Chấp nhận Rủi ro của các Dịch vụ

Mức độ chấp nhận rủi ro của một dịch vụ được hiểu như thế nào? Trong môi trường chính thức hoặc với các hệ thống an toàn quan trọng, mức độ này thường được tích hợp trực tiếp vào định nghĩa cốt lõi của sản phẩm hoặc dịch vụ. Tại Google, mức độ chấp nhận rủi ro của các dịch vụ lại thường được xác định kém rõ ràng hơn.
...
Các dịch vụ người tiêu dùng của chúng tôi thường có một đội sản phẩm giữ vai trò chủ sở hữu business cho ứng dụng. Chẳng hạn, Search, Google Maps và Google Docs mỗi thứ đều có đội product manager (quản lý sản phẩm) riêng. Nhiệm vụ của các product manager này là hiểu người dùng và business, đồng thời định hình sản phẩm để thành công trên thị trường. Khi có đội sản phẩm, đây thường là nguồn tốt nhất để bàn về các yêu cầu độ tin cậy của dịch vụ. Nếu không có đội sản phẩm chuyên trách, các kỹ sư xây dựng hệ thống thường vô tình đảm nhận vai trò này.
...
Mức khả dụng mục tiêu của một dịch vụ Google cụ thể thường phụ thuộc vào chức năng mà dịch vụ đó cung cấp và cách nó được định vị trên thị trường. Danh sách sau liệt kê các vấn đề cần xem xét:
...
Hãy xét trường hợp của Google Apps for Work. Phần lớn người dùng của dịch vụ này là doanh nghiệp, quy mô có lớn có nhỏ. Họ dựa vào các dịch vụ Google Apps for Work (ví dụ Gmail, Calendar, Drive, Docs) để cung cấp công cụ hỗ trợ nhân viên làm việc hàng ngày. Nói cách khác, một outage của Google Apps for Work không chỉ là outage của Google mà còn của tất cả các doanh nghiệp phụ thuộc quan trọng vào chúng tôi. Với một dịch vụ Google Apps for Work điển hình, chúng tôi có thể đặt mục tiêu khả dụng hàng quý bên ngoài là 99.9%, và củng cố mục tiêu này bằng một mục tiêu khả dụng nội bộ cao hơn cùng một hợp đồng quy định hình phạt nếu chúng tôi không đạt được mục tiêu bên ngoài.

YouTube là một trường hợp đối lập. Khi Google mua YouTube, chúng tôi phải xác định mục tiêu khả dụng phù hợp cho website. Năm 2006, YouTube hướng đến người tiêu dùng và đang ở giai đoạn vòng đời kinh doanh rất khác so với Google lúc bấy giờ. Dù đã có một sản phẩm tuyệt vời, YouTube vẫn đang thay đổi và phát triển nhanh. Chúng tôi đặt mục tiêu khả dụng thấp hơn cho YouTube so với các sản phẩm doanh nghiệp vì lúc đó tốc độ phát triển tính năng quan trọng tương ứng hơn.

### Các loại sự cố

Hình dạng dự kiến của các sự cố với một dịch vụ cụ thể cũng là một cân nhắc quan trọng. Business của chúng tôi chống chịu thế nào trước downtime dịch vụ? Với dịch vụ, cái nào tệ hơn: một tỷ lệ thất bại thấp nhưng kéo dài, hay một outage toàn site thỉnh thoảng? Cả hai loại có thể dẫn đến cùng một số lỗi tuyệt đối, nhưng tác động đến business lại rất khác nhau.

Một ví dụ điển hình cho thấy sự khác biệt giữa outage toàn bộ và cục bộ thường gặp trong các hệ thống phục vụ thông tin cá nhân. Hãy xét một ứng dụng quản lý liên hệ, và so sánh hai tình huống: sự cố khiến ảnh hồ sơ không hiển thị được, và sự cố khiến danh bạ cá nhân của một người dùng bị hiển thị cho người dùng khác. Trường hợp đầu tiên rõ ràng là trải nghiệm người dùng kém, và các SRE sẽ nỗ lực khắc phục nhanh. Trường hợp thứ hai, rủi ro phơi bày dữ liệu cá nhân có thể làm suy yếu nghiêm trọng niềm tin cơ bản của người dùng. Vì vậy, đối với trường hợp thứ hai, việc đưa dịch vụ offline hoàn toàn trong giai đoạn debug và dọn dẹp là hợp lý.

Ở đầu kia của phổ dịch vụ mà Google cung cấp, đôi khi người ta chấp nhận được việc ngừng dịch vụ định kỳ trong các cửa sổ bảo trì (maintenance window). Cách đây vài năm, Ads Frontend từng là một dịch vụ như vậy. Các nhà quảng cáo và nhà xuất bản website dùng nó để thiết lập, cấu hình, chạy và giám sát các chiến dịch quảng cáo. Vì phần lớn công việc này diễn ra trong giờ hành chính, chúng tôi xác định rằng các sự cố ngừng dịch vụ định kỳ, đều đặn, được lên kế hoạch dưới dạng cửa sổ bảo trì là có thể chấp nhận được, và chúng tôi xếp những sự cố này vào downtime có kế hoạch (planned downtime) chứ không phải downtime không có kế hoạch.

### Chi phí

Chi phí thường là yếu tố then chốt khi xác định mục tiêu khả dụng phù hợp cho một dịch vụ. Với Ads, việc đánh đổi này đặc biệt thuận lợi vì kết quả thành công hay thất bại có thể quy đổi trực tiếp thành doanh thu thu được hoặc mất đi. Khi xác định mục tiêu khả dụng cho từng dịch vụ, chúng tôi đặt ra các câu hỏi như:
...
Trong trường hợp này, nếu chi phí cải thiện khả dụng một nine thấp hơn $900, việc đầu tư là đáng giá. Ngược lại, nếu chi phí vượt quá $900, nó sẽ lớn hơn mức tăng doanh thu dự kiến.

Việc đặt các mục tiêu này có thể khó hơn khi chúng tôi không có một hàm quy đổi đơn giản giữa độ tin cậy và doanh thu. Một chiến lược hữu ích là xem xét tỷ lệ lỗi nền của các ISP (nhà cung cấp dịch vụ Internet) trên Internet. Nếu sự cố được đo từ góc nhìn người dùng cuối và ta có thể đẩy tỷ lệ lỗi của dịch vụ xuống dưới tỷ lệ lỗi nền, thì những lỗi đó sẽ rơi vào phần nhiễu của kết nối Internet cho một người dùng cụ thể. Dù có khác biệt đáng kể giữa các ISP và giao thức (ví dụ TCP so với UDP, IPv4 so với IPv6), chúng tôi đo được tỷ lệ lỗi nền điển hình của các ISP nằm trong khoảng từ 0,01% đến 1%.

### Các metrics dịch vụ khác

Đánh giá mức độ chấp nhận rủi ro của các dịch vụ dựa trên các metrics ngoài khả dụng thường mang lại nhiều lợi ích. Khi nắm rõ metrics nào quan trọng và metrics nào không, chúng tôi có nhiều tự do hơn trong việc chấp nhận rủi ro có suy xét.

Độ trễ dịch vụ (service latency) của các hệ thống Ads là một ví dụ minh họa. Khi Google ra mắt Web Search, tốc độ là một trong những tính năng phân biệt chính của dịch vụ. Khi giới thiệu AdWords — hiển thị quảng cáo bên cạnh kết quả tìm kiếm — một yêu cầu chính của hệ thống là quảng cáo không được làm chậm trải nghiệm tìm kiếm. Yêu cầu này đã định hình các mục tiêu kỹ thuật qua từng thế hệ hệ thống AdWords và được xem là một bất biến (invariant).

AdSense, hệ thống quảng cáo của Google, phục vụ các quảng cáo ngữ cảnh (contextual ads) để đáp lại các yêu cầu từ code JavaScript mà nhà xuất bản chèn vào website, nhưng lại có mục tiêu độ trễ rất khác. Mục tiêu của AdSense là tránh làm chậm việc render (hiển thị) của trang bên thứ ba khi chèn quảng cáo ngữ cảnh. Do đó, mục tiêu độ trễ cụ thể phụ thuộc vào tốc độ render trang của từng nhà xuất bản. Điều này có nghĩa là quảng cáo AdSense nhìn chung có thể được phục vụ chậm hơn quảng cáo AdWords hàng trăm mili giây.

Việc nới lỏng yêu cầu về độ trễ cho phép chúng tôi thực hiện nhiều đánh đổi thông minh trong khâu cấp phát tài nguyên — tức xác định số lượng và vị trí các tài nguyên phục vụ — giúp tiết kiệm đáng kể chi phí so với cách cấp phát mặc định. Nói cách khác, vì AdSense không quá nhạy cảm với các thay đổi độ trễ ở mức vừa phải, chúng tôi có thể gộp việc phục vụ vào ít vị trí địa lý hơn, từ đó giảm chi phí vận hành.

## Xác định Mức độ Chấp nhận Rủi ro của các Dịch vụ Cơ sở Hạ tầng

Việc xây dựng và vận hành các thành phần cơ sở hạ tầng có những điểm khác biệt so với sản phẩm người tiêu dùng. Một khác biệt cơ bản là, theo định nghĩa, các thành phần cơ sở hạ tầng luôn phục vụ nhiều client (khách hàng) với các nhu cầu thường khác nhau.

### Mức khả dụng mục tiêu

Hãy xem xét Bigtable [[Cha06]](https://sre.google/sre-book/bibliography#Cha06), một hệ thống lưu trữ phân tán quy mô khổng lồ cho dữ liệu có cấu trúc. Một số dịch vụ người tiêu dùng phục vụ dữ liệu trực tiếp từ Bigtable trong đường đi của một yêu cầu. Những dịch vụ này cần độ trễ thấp và độ tin cậy cao. Các đội khác dùng Bigtable như một repository (kho dữ liệu) cho dữ liệu phục vụ phân tích offline (ví dụ MapReduce) định kỳ. Những đội này thường quan tâm nhiều hơn đến thông lượng (throughput) hơn là độ tin cậy. Mức độ chấp nhận rủi ro của hai use case (tình huống sử dụng) này khá khác nhau.

Một cách để đáp ứng nhu cầu của cả hai use case là đầu tư kỹ thuật để đưa mọi dịch vụ cơ sở hạ tầng lên mức độ tin cậy cực kỳ cao. Nhưng vì các dịch vụ cơ sở hạ tầng này thường gom lại một lượng lớn tài nguyên, cách làm đó trong thực tế thường đắt đỏ hơn rất nhiều. Để hiểu nhu cầu khác nhau của các loại người dùng, bạn có thể xem xét trạng thái mong muốn của hàng đợi yêu cầu (request queue) cho mỗi loại người dùng Bigtable.

### Các loại thất bại

Người dùng độ trễ thấp muốn hàng đợi yêu cầu của Bigtable (gần như luôn) trống để hệ thống xử lý mỗi yêu cầu ngay khi đến. (Thực tế, xếp hàng kém hiệu quả thường là nguyên nhân gây độ trễ tail cao.) Ngược lại, người dùng quan tâm đến phân tích offline lại ưu tiên throughput của hệ thống, nên họ muốn hàng đợi yêu cầu không bao giờ trống. Để tối ưu throughput, hệ thống Bigtable không nên để idle (nghỉ) chờ yêu cầu tiếp theo.

Như bạn thấy, với các nhóm người dùng này, thành công và thất bại là hai mặt đối lập. Điều mang lại thành công cho người dùng độ trễ thấp lại là thất bại đối với người dùng quan tâm đến phân tích offline.

### Chi phí

Một cách tiết kiệm để thỏa mãn các ràng buộc cạnh tranh này là phân chia (partition) cơ sở hạ tầng và cung cấp ở nhiều mức dịch vụ độc lập. Trong ví dụ Bigtable, chúng tôi có thể xây dựng hai loại cluster (cụm máy): cluster độ trễ thấp và cluster throughput. Cluster độ trễ thấp được thiết kế cho các dịch vụ cần độ trễ thấp và độ tin cậy cao. Để đảm bảo hàng đợi ngắn và đáp ứng các yêu cầu cách ly client khắt khe hơn, hệ thống Bigtable có thể được provision với một lượng lớn slack capacity (năng lực dự phòng), giúp giảm cạnh tranh và tăng dự phòng. Ngược lại, cluster throughput có thể được provision để chạy rất nóng với ít dự phòng hơn, đặt throughput lên trên độ trễ. Trên thực tế, chúng tôi có thể đáp ứng các nhu cầu nới lỏng này với chi phí thấp hơn nhiều, có khi chỉ 10–50% chi phí của một cluster độ trễ thấp. Với quy mô khổng lồ của Bigtable, khoản tiết kiệm này trở nên đáng kể rất nhanh.

Chiến lược then chốt với cơ sở hạ tầng là cung cấp các dịch vụ có các mức dịch vụ được phân định rõ ràng, giúp các client đưa ra đánh đổi giữa rủi ro và chi phí một cách chính xác khi xây dựng hệ thống. Khi các mức dịch vụ đã rõ ràng, nhà cung cấp cơ sở hạ tầng có thể chuyển (externalize) sự khác biệt về chi phí cung cấp dịch vụ ở từng mức sang phía client. Việc phơi bày chi phí theo cách này tạo động lực để client chọn mức dịch vụ rẻ nhất mà vẫn đáp ứng nhu cầu. Ví dụ, Google+ có thể đặt dữ liệu quan trọng để thực thi quyền riêng tư người dùng trong một datastore (kho dữ liệu) khả dụng cao, nhất quán toàn cầu (ví dụ một hệ thống giống SQL được sao chép toàn cầu như Spanner [[Cor12]](https://sre.google/sre-book/bibliography#Cor12)), trong khi đặt dữ liệu tùy chọn (không quan trọng nhưng cải thiện trải nghiệm người dùng) trong một datastore rẻ hơn, kém tin cậy hơn, kém tươi hơn và eventually consistent (ví dụ một kho NoSQL dùng sao chép best-effort (nỗ lực tốt nhất) như Bigtable).

Lưu ý rằng chúng tôi có thể vận hành nhiều lớp dịch vụ trên phần cứng và phần mềm giống hệt nhau. Bằng cách điều chỉnh một loạt đặc tính dịch vụ — số lượng tài nguyên, mức độ dự phòng, ràng buộc provision địa lý và, quan trọng nhất, cấu hình phần mềm cơ sở hạ tầng — chúng tôi có thể đưa ra các đảm bảo dịch vụ rất khác nhau.

### Ví dụ: Cơ sở hạ tầng frontend

Để thấy các nguyên lý đánh giá mức độ chấp nhận rủi ro này không chỉ áp dụng cho cơ sở hạ tầng lưu trữ, hãy xem một lớp dịch vụ lớn khác: cơ sở hạ tầng frontend của Google. Cơ sở hạ tầng frontend gồm các hệ thống reverse proxy (proxy ngược) và load balancing (cân bằng tải) chạy gần mép (edge) của mạng. Đây là các hệ thống đảm nhận nhiều vai trò, trong đó có làm một đầu cuối của các kết nối từ người dùng cuối (ví dụ terminate (kết thúc) TCP từ trình duyệt của người dùng). Vì vai trò quan trọng của chúng, chúng tôi đầu tư công sức kỹ thuật để đưa các hệ thống này lên độ tin cậy cực kỳ cao. Trong khi các dịch vụ người tiêu dùng thường có thể che bớt sự không tin cậy ở backend, các hệ thống cơ sở hạ tầng này không may mắn như vậy. Nếu một yêu cầu không bao giờ đến được frontend server của dịch vụ ứng dụng, nó bị mất.
...
Những căng thẳng này thường bộc lộ qua các quan điểm khác nhau về mức độ nỗ lực cần đầu tư cho các thực hành kỹ thuật. Dưới đây là một số căng thẳng điển hình:
...
Một best practice là kiểm thử release mới trên một tập con nhỏ của workload (tải công việc) điển hình — thực hành thường gọi là *canarying* (chạy thử nghiệm trên một phần nhỏ). Chúng ta chờ bao lâu, và canary lớn cỡ nào?

Thông thường, các đội hiện có đã tự tìm ra một dạng cân bằng không chính thức về ranh giới rủi ro/nỗ lực. Thật không may, hiếm khi có thể chứng minh cân bằng này là tối ưu; đó đơn giản là một hàm của kỹ năng đàm phán của các kỹ sư liên quan. Và các quyết định như vậy cũng không nên để chính trị, sợ hãi hay hy vọng dẫn dắt. (Quả vậy, khẩu hiệu không chính thức của Google SRE là "Hope is not a strategy" — Hy vọng không phải là một chiến lược.) Thay vào đó, mục tiêu của chúng tôi là định nghĩa một metrics khách quan, được cả hai bên đồng ý, có thể dùng để dẫn dắt các cuộc đàm phán theo cách có thể tái lập. Quyết định càng dựa trên dữ liệu, càng tốt.

## Hình thành Error Budget của Bạn

Để các quyết định này dựa trên dữ liệu khách quan, hai đội cùng định nghĩa một error budget hàng quý dựa trên service level objective (SLO) của dịch vụ (xem [Service Level Objectives](04-service-level-objectives.md)). Error budget là một metrics rõ ràng, khách quan, xác định mức độ không tin cậy tối đa mà dịch vụ được phép có trong một quý. Metrics này giúp loại bỏ yếu tố chính trị khỏi các cuộc đàm phán giữa SRE và developer sản phẩm khi quyết định mức rủi ro chấp nhận được.
...
Ví dụ, giả sử SLO của một dịch vụ là xử lý thành công 99.999% tổng số query (yêu cầu truy vấn) mỗi quý. Điều này có nghĩa là error budget của dịch vụ tương ứng với tỷ lệ thất bại 0.001% trong một quý nhất định. Nếu một sự cố khiến 0.0002% số query dự kiến trong quý bị lỗi, sự cố đó đã tiêu tốn 20% error budget hàng quý của dịch vụ.

## Lợi ích

Lợi ích chính của error budget là tạo ra động lực chung, giúp cả đội phát triển sản phẩm lẫn SRE cùng tìm được sự cân bằng phù hợp giữa đổi mới và độ tin cậy.
...
Ví dụ, nếu đội phát triển sản phẩm muốn cắt giảm kiểm thử hoặc đẩy nhanh tốc độ push trong khi SRE phản đối, error budget sẽ là cơ sở để ra quyết định. Khi ngân sách còn dư dả, developer sản phẩm có thể chấp nhận rủi ro cao hơn. Ngược lại, khi ngân sách sắp cạn, chính họ sẽ chủ động tăng cường kiểm thử hoặc giảm tốc độ push, nhằm tránh việc dùng hết ngân sách và làm chậm tiến độ ra mắt. Về mặt hiệu lực, đội phát triển sản phẩm trở thành tự giám sát (self-policing): họ nắm rõ ngân sách và có khả năng tự quản lý rủi ro. (Tất nhiên, điều này dựa trên việc đội SRE thực sự có quyền dừng các launch (ra mắt) nếu SLO bị vi phạm.)

Nếu xảy ra sự cố mạng hoặc sự cố datacenter (trung tâm dữ liệu) khiến SLO đo được giảm, điều gì sẽ xảy ra? Những sự kiện này cũng làm xói mòn error budget. Hệ quả là, số push mới có thể bị giảm trong phần còn lại của quý. Toàn bộ đội đều ủng hộ việc giảm này vì ai cũng chung trách nhiệm về uptime.
...
-   Error budget cân bằng các động lực và nhấn mạnh sự sở hữu chung giữa SRE và phát triển sản phẩm. Nó giúp việc quyết định tốc độ release dễ dàng hơn, gỡ rối các cuộc thảo luận về outage với các bên liên quan hiệu quả hơn, và cho phép nhiều đội cùng đi đến một kết luận về rủi ro production mà không nảy sinh bất hòa.
