...
Bao gồm các việc như chạy thủ công một script (chương trình) tự động hóa một số tác vụ. Chạy script có thể nhanh hơn làm thủ công từng bước trong script, nhưng khoảng thời gian *thực hiện bằng tay* (hands-on) mà một người bỏ ra để chạy script đó (không phải thời gian trôi qua) vẫn là toil.
...
Toil xuất phát từ sự gián đoạn (interrupt-driven) và mang tính phản ứng (reactive), chứ không phải xuất phát từ chiến lược (strategy-driven) và chủ động (proactive). Xử lý các cảnh báo pager (thiết bị gọi trực) là toil. Chúng tôi có thể không bao giờ loại bỏ hoàn toàn loại công việc này, nhưng phải liên tục hướng đến việc giảm thiểu nó.
...
Hơn nữa, khi tuyển SRE mới, chúng tôi hứa với họ rằng SRE không phải là một tổ chức Ops điển hình, viện dẫn quy tắc 50% vừa nêu. Chúng tôi cần giữ lời hứa đó bằng cách không để tổ chức SRE hay bất kỳ đội con nào thoái hóa thành một đội Ops.
...
Toil quá mức khiến một đội kém năng suất hơn. Feature velocity (tốc độ tính năng) của một sản phẩm sẽ chậm lại nếu đội SRE quá bận với việc thủ công và chữa cháy (firefighting) đến mức không kịp triển khai các tính năng mới.
...
Ngay cả khi bạn cá nhân không bất hạnh với toil, các đồng đội hiện tại hoặc tương lai của bạn có thể thích nó ít hơn nhiều. Nếu bạn xây dựng quá nhiều toil vào các quy trình của đội, bạn tạo động lực cho các kỹ sư giỏi nhất của đội bắt đầu tìm một công việc xứng đáng hơn ở nơi khác.
