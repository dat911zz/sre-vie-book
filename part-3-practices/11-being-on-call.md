...
Các sự kiện production không paging, chẳng hạn các cảnh báo ưu tiên thấp hơn hoặc các release (phát hành) phần mềm, cũng có thể được xử lý và/hoặc xem xét bởi kỹ sư on-call trong giờ làm việc. Những hoạt động này ít khẩn cấp hơn các sự kiện paging — thứ được ưu tiên gần như mọi tác vụ khác, kể cả công việc dự án. Để hiểu thêm về các ngắt (interrupt) và các sự kiện không paging khác góp phần vào khối lượng công việc vận hành, xem [Dealing with Interrupts](https://sre.google/sre-book/dealing-with-interrupts/).
...
Áp dụng quy tắc 25% on-call, chúng tôi có thể suy ra số lượng tối thiểu SRE cần để duy trì một vòng on-call 24/7. Giả sử luôn có hai người on-call (chính và phụ, nhiệm vụ khác nhau), số kỹ sư tối thiểu cần cho nhiệm vụ on-call của một đội đơn vị (single-site) là tám: với ca tính theo tuần, mỗi kỹ sư on-call (chính hoặc phụ) một tuần mỗi tháng. Với các đội hai vị trí (dual-site), kích thước tối thiểu hợp lý của mỗi đội là sáu, vừa đáp ứng quy tắc 25% vừa đảm bảo đủ số lượng và khối lượng tới hạn (critical mass) kỹ sư cho đội.

Nếu một dịch vụ có đủ công việc để hợp lý hóa việc thành lập một đội đơn vị, chúng tôi vẫn thích tạo một đội đa vị trí (multi-site). Đội đa vị trí có lợi thế về hai mặt:

-   Ca đêm có hại cho sức khỏe con người [[Dur05]](https://sre.google/sre-book/bibliography#Dur05), và một vòng on-call "chase the sun" (đuổi theo mặt trời) đa vị trí cho phép các đội tránh hoàn toàn ca đêm.
...
Các đội developer (phát triển) của những hệ thống do SRE hỗ trợ thường tham gia một vòng on-call 24/7, và luôn có thể leo thang đến các đội đối tác này khi cần. Việc leo thang outage một cách đúng lúc nhìn chung là cách có nguyên tắc để phản ứng với các outage nghiêm trọng có nhiều yếu tố chưa rõ.

Khi xử lý incident, nếu vấn đề đủ phức tạp để kéo nhiều đội vào, hoặc nếu sau một số điều tra vẫn chưa ước lượng được giới hạn trên của thời gian kéo dài incident, việc áp dụng một giao thức quản lý incident chính thức là hữu ích. Google SRE dùng giao thức mô tả trong [Managing Incidents](https://sre.google/sre-book/managing-incidents/), gồm một tập các bước dễ theo dõi, định nghĩa rõ ràng, giúp kỹ sư on-call theo đuổi một cách hợp lý một giải pháp incident thỏa mãn với đầy đủ sự hỗ trợ cần thiết. Giao thức này được hỗ trợ nội bộ bởi một công cụ web tự động hóa phần lớn hành động quản lý incident, như bàn giao vai trò và ghi lại, truyền thông các cập nhật trạng thái. Công cụ này cho phép người quản lý incident tập trung vào việc xử lý incident, thay vì mất thời gian và công sức nhận thức cho những việc vụn vặt như định dạng email hay cập nhật nhiều kênh truyền thông cùng lúc.
...
Lý tưởng nhất, các triệu chứng của việc chở quá nhiều công việc vận hành nên đo được, để mục tiêu có thể định lượng (ví dụ, số ticket mỗi ngày < 5, sự kiện paging mỗi ca < 2).

Cấu hình giám sát sai là nguyên nhân phổ biến của việc chở quá nhiều công việc vận hành. Các cảnh báo paging nên được căn chỉnh với các triệu chứng đe dọa SLO của dịch vụ. Mọi cảnh báo paging cũng phải có thể hành động được (actionable). Các cảnh báo ưu tiên thấp làm phiền kỹ sư on-call mỗi giờ (hoặc thường xuyên hơn) gây gián đoạn năng suất, và sự mệt mỏi chúng gây ra có thể khiến các cảnh báo nghiêm trọng bị xử lý với ít sự chú ý hơn mức cần. Xem [Dealing with Interrupts](https://sre.google/sre-book/dealing-with-interrupts/) để thảo luận thêm.
