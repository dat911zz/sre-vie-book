...
Về lý tưởng, các triệu chứng của việc chở quá nhiều công việc vận hành nên đo được, để mục tiêu có thể định lượng (ví dụ, số ticket mỗi ngày < 5, sự kiện paging mỗi ca < 2).

Cấu hình giám sát sai là nguyên nhân phổ biến của việc chở quá nhiều công việc vận hành. Các page nên được căn chỉnh với các triệu chứng đe dọa SLO của dịch vụ. Mọi page cũng phải có thể hành động được (actionable). Các cảnh báo ưu tiên thấp làm phiền kỹ sư on-call mỗi giờ (hoặc thường xuyên hơn) gây gián đoạn năng suất, và sự mệt mỏi chúng gây ra có thể khiến các cảnh báo nghiêm trọng bị xử lý với ít sự chú ý hơn mức cần. Xem [Dealing with Interrupts](https://sre.google/sre-book/dealing-with-interrupts/) để thảo luận thêm.
