...
Ngay khi nhận và xác nhận một lần gọi trực, kỹ sư on-call được kỳ vọng sẽ phân loại (triage) vấn đề và tiến hành giải quyết, có thể kéo các thành viên khác trong đội vào và leo thang (escalate) khi cần.

Kỹ sư on-call cũng có thể xử lý và/hoặc xem xét trong giờ làm việc các sự kiện production ngoài diện gọi trực, chẳng hạn các cảnh báo ưu tiên thấp hơn hoặc các release (phát hành) phần mềm. Những hoạt động này ít khẩn cấp hơn các sự kiện gọi trực — thứ được ưu tiên hơn gần như mọi tác vụ khác, kể cả công việc dự án. Để hiểu thêm về các gián đoạn (interrupt) và các sự kiện ngoài diện gọi trực khác góp phần vào khối lượng công việc vận hành, xem [Dealing with Interrupts](https://sre.google/sre-book/dealing-with-interrupts/).

Nhiều đội có cả vòng on-call chính (primary) và phụ (secondary). Cách phân chia nhiệm vụ giữa chính và phụ khác nhau tùy đội. Đội này có thể dùng người phụ làm điểm rơi (fall-through) cho các lần gọi trực mà on-call chính bỏ lỡ. Đội khác có thể quy định on-call chính chỉ xử lý gọi trực, còn người phụ đảm nhận mọi hoạt động production khẩn cấp khác.
...
Về lý tưởng, các triệu chứng của việc chở quá nhiều công việc vận hành nên đo được, để mục tiêu có thể định lượng (ví dụ, số ticket mỗi ngày < 5, sự kiện gọi trực mỗi ca < 2).

Cấu hình giám sát sai là nguyên nhân phổ biến của việc chở quá nhiều công việc vận hành. Các cảnh báo gọi trực nên được căn chỉnh với các triệu chứng đe dọa SLO của dịch vụ. Mọi cảnh báo gọi trực cũng phải có thể hành động được (actionable). Các cảnh báo ưu tiên thấp làm phiền kỹ sư on-call mỗi giờ (hoặc thường xuyên hơn) gây gián đoạn năng suất, và sự mệt mỏi chúng gây ra có thể khiến các cảnh báo nghiêm trọng bị xử lý với ít sự chú ý hơn mức cần. Xem [Dealing with Interrupts](https://sre.google/sre-book/dealing-with-interrupts/) để thảo luận thêm.
...
Trong các trường hợp cực đoan, các đội SRE có thể có tùy chọn "trả lại máy gọi trực" — SRE có thể yêu cầu đội developer on-call độc quyền cho hệ thống cho đến khi nó đáp ứng các tiêu chuẩn của đội SRE. Việc trả lại máy gọi trực không xảy ra thường xuyên, vì gần như luôn có thể làm việc với đội developer để giảm khối lượng công việc vận hành và làm hệ thống cụ thể đó đáng tin cậy hơn. Tuy nhiên, trong một số trường hợp, các thay đổi phức tạp hoặc kiến trúc trải qua nhiều quý có thể cần thiết để làm hệ thống bền vững từ góc độ vận hành. Trong những trường hợp đó, đội SRE không nên phải chịu một khối lượng công việc vận hành quá mức. Thay vào đó, hợp lý là đàm phán tái tổ chức trách nhiệm on-call với đội phát triển, có thể định tuyến một số hoặc tất cả cảnh báo gọi trực đến developer on-call. Giải pháp như vậy thường là biện pháp tạm thời, trong khoảng thời gian đó các đội SRE và developer làm việc cùng nhau để đưa dịch vụ vào trạng thái có thể được SRE onboard (tiếp nhận) lại.

Khả năng đàm phán lại trách nhiệm on-call giữa các đội SRE và phát triển sản phẩm cho thấy sự cân bằng quyền lực giữa các đội.<sup>[2](#fn2)</sup> Mối quan hệ làm việc này cũng là ví dụ điển hình cho cách căng thẳng lành mạnh giữa hai đội — và các giá trị họ đại diện, độ tin cậy so với feature velocity — thường được giải quyết bằng cách mang lại lợi ích lớn cho dịch vụ và, qua đó, cho toàn bộ công ty.
