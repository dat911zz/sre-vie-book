...
Quản lý incident (sự cố) hiệu quả là chìa khóa để hạn chế sự gián đoạn mà incident gây ra và khôi phục hoạt động business bình thường nhanh nhất có thể. Nếu bạn không mô phỏng (game out) trước [phản ứng của mình với các incident tiềm tàng](https://sre.google/resources/practices-and-processes/anatomy-of-an-incident/), cách quản lý incident có nguyên tắc có thể bị phá vỡ ngay trong các tình huống thực tế.
...
Thời gian trôi qua; hai datacenter còn lại cũng hỏng hoàn toàn. Trong khi bạn không hay biết, Josephine đang mê mệt giấc ngủ đã gọi cho Malcolm. Anh ấy lóe lên một ý tưởng: có gì đó liên quan đến CPU affinity (độ bám dính CPU). Anh tin chắc mình có thể tối ưu hóa các tiến trình server còn lại nếu chỉ cần triển khai một thay đổi đơn giản này xuống môi trường production, nên anh đã làm vậy. Chỉ trong vài giây, các server khởi động lại, nạp thay đổi. Rồi chết.
...
Các bên liên quan cần hiểu nơi họ có thể tương tác với người chỉ huy incident. Trong nhiều tình huống, việc tập hợp các thành viên lực lượng đặc nhiệm incident vào một "War Room" (Phòng chỉ huy) trung tâm là hợp lý. Các đội khác có thể thích làm việc tại chỗ, theo dõi các cập nhật incident qua email và IRC (Internet Relay Chat — Trò chuyện Trung chuyển Internet).

Google nhận thấy IRC là một lợi ích lớn trong phản ứng sự cố. IRC rất đáng tin cậy và có thể dùng như một log các trao đổi về sự kiện, một bản ghi như vậy vô giá trong việc giữ các thay đổi trạng thái chi tiết trong tâm trí. Chúng tôi cũng viết các bot (robot) ghi log traffic liên quan đến incident (hữu ích cho phân tích postmortem) và các bot khác ghi log các sự kiện như cảnh báo đến kênh. IRC cũng là môi trường thuận tiện để các đội phân tán về mặt địa lý phối hợp với nhau.
...
-   Outage (mất dịch vụ) có nhìn thấy được bởi khách hàng không?
