...
Để cải thiện độ tin cậy theo thời gian, bạn cần một baseline (mốc chuẩn) rõ ràng và khả năng theo dõi tiến bộ. Outalator, công cụ theo dõi sự cố ngừng dịch vụ của chúng tôi, là một trong những phương tiện giúp thực hiện chính xác điều đó. Đây là hệ thống thụ động, nhận tất cả cảnh báo từ các hệ thống giám sát, cho phép chúng tôi chú thích, nhóm và phân tích dữ liệu này.

Học hỏi có hệ thống từ các vấn đề trong quá khứ là điều thiết yếu để quản lý dịch vụ hiệu quả. Các postmortem (báo cáo sau sự cố) (xem [Postmortem Culture: Learning from Failure](https://sre.google/sre-book/postmortem-culture/)) cung cấp chi tiết cho từng outage riêng lẻ, nhưng chỉ là một phần câu trả lời. Vì chúng chỉ được viết cho các incident (sự cố) có tác động lớn, nên những vấn đề có tác động nhỏ lẻ nhưng thường xuyên và phổ biến sẽ không nằm trong phạm vi. Tương tự, postmortem có xu hướng mang lại những hiểu biết sâu sắc (insight) hữu ích để cải thiện một dịch vụ hay một nhóm dịch vụ đơn lẻ, nhưng có thể bỏ lỡ các cơ hội có hiệu ứng nhỏ trong từng trường hợp, hoặc những cơ hội có tỷ lệ chi phí/lợi ích (cost/benefit) thấp nhưng lại có tác động ngang (horizontal) lớn.<sup>[1](#fn1)</sup>
...
Outalator cho phép người dùng xem danh sách các thông báo từ nhiều hàng đợi (queue) đan xen theo thời gian, thay vì phải chuyển đổi thủ công giữa các hàng đợi. [Hình 16-1](#hinh-16-1) hiển thị nhiều hàng đợi như chúng xuất hiện trong chế độ xem hàng đợi của Outalator. Tính năng này rất tiện lợi, vì một đội SRE thường là điểm liên lạc chính cho nhiều dịch vụ, mỗi dịch vụ lại có các điểm leo thang phụ riêng, thường thuộc về các đội developer (phát triển).
...
Trong Outalator, nhiều thông báo leo thang ("cảnh báo") có thể được gộp thành một thực thể duy nhất ("incident"). Các thông báo này có thể cùng thuộc một incident, hoặc không liên quan và chỉ là những sự kiện ít đáng chú ý nhưng vẫn kiểm toán (auditable) được, chẳng hạn như truy cập database đặc quyền, hoặc đơn giản là các sự cố giám sát giả. Tính năng nhóm này, được minh họa trong [Hình 16-2](#hinh-16-2), giúp giảm sự lộn xộn trên các màn hình tổng quan và cho phép so sánh riêng "số incident mỗi ngày" với "số cảnh báo mỗi ngày".
...
Một sự kiện đơn lẻ có thể, và thường sẽ, kích hoạt nhiều cảnh báo. Chẳng hạn, một sự cố mạng gây ra các timeout (thời gian chờ hết) và khiến dịch vụ backend (phía sau) không truy cập được, dẫn đến việc mọi đội bị ảnh hưởng đều nhận cảnh báo của riêng mình, bao gồm cả chủ sở hữu dịch vụ backend; trong khi đó, trung tâm vận hành mạng (network operations center) lại có hẳn các còi báo động (klaxon) của nó. Ngay cả các vấn đề nhỏ hơn chỉ ảnh hưởng đến một dịch vụ đơn lẻ cũng có thể kích hoạt nhiều cảnh báo do nhiều điều kiện lỗi được chẩn đoán. Dù đáng cố gắng tối thiểu hóa số cảnh báo do một sự kiện đơn lẻ kích hoạt, việc kích hoạt nhiều cảnh báo là không thể tránh khỏi trong hầu hết các phép đánh đổi giữa dương tính giả (false positive) và âm tính giả (false negative).

Việc gom nhiều cảnh báo thành một *incident* (sự cố) duy nhất là điều thiết yếu để giải quyết tình trạng trùng lặp này. Gửi một email với nội dung “đây là cùng một thứ với thứ kia; chúng là triệu chứng của cùng một incident” sẽ hữu ích cho một cảnh báo cụ thể: nó có thể ngăn việc debug (xử lý lỗi) hoặc hoảng loạn lặp lại. Tuy nhiên, gửi một email cho mỗi cảnh báo không phải là giải pháp thực tế hay có thể mở rộng (scale) để xử lý cảnh báo trùng lặp trong một đội, chứ đừng nói đến giữa các đội hay trên các khoảng thời gian dài hơn.

## Dán Nhãn (Tagging)

Tất nhiên, không phải mọi sự kiện cảnh báo đều là một incident. Các cảnh báo dương tính giả, sự kiện kiểm thử hay email gửi sai người do lỗi con người đều có thể xảy ra. Bản thân Outalator không phân biệt những sự kiện này, nhưng cho phép *dán nhãn* (tagging) tổng quát để thêm metadata (dữ liệu mô tả) vào các thông báo ở bất kỳ cấp nào. Các tag (nhãn) phần lớn tự do, là những "từ" đơn lẻ. Tuy nhiên, dấu hai chấm được diễn giải như dấu phân tách ngữ nghĩa (semantic separator), điều này tinh tế khuyến khích dùng các không gian tên phân cấp (hierarchical namespace) và cho phép một số xử lý tự động. Các tiền tố tag (tag prefix) được đề xuất — chủ yếu là "cause" (nguyên nhân) và "action" (hành động) — giúp định hình không gian tên này, nhưng danh sách tiền tố cụ thể do từng đội tự xây dựng dựa trên cách dùng trong quá khứ. Ví dụ, "cause:network" (nguyên nhân:mạng) có thể đủ thông tin cho một số đội, trong khi một đội khác có thể chọn tag cụ thể hơn, như "cause:network:switch" (nguyên nhân:mạng:bộ chuyển mạch) so với "cause:network:cable" (nguyên nhân:mạng:dây cáp). Một số đội thường xuyên dùng tag kiểu "customer:132456" (khách hàng:132456), nên "customer" (khách hàng) sẽ được đề xuất cho những đội đó, nhưng không cho đội khác.
