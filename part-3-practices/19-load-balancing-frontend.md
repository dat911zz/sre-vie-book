...
Vấn đề đầu tiên là cách này cho phép kiểm soát hành vi client rất hạn chế: các bản ghi được chọn ngẫu nhiên, và mỗi bản sẽ thu hút một lượng traffic xấp xỉ bằng nhau. Chúng tôi có thể giảm nhẹ vấn đề này không? Về mặt lý thuyết, chúng tôi có thể dùng các bản ghi `SRV` để chỉ định trọng số và thứ tự ưu tiên, nhưng các bản ghi `SRV` chưa được áp dụng cho HTTP.
