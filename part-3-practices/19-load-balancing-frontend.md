...
Trong thực tế, Google có hàng nghìn máy và còn nhiều người dùng hơn nữa, trong đó nhiều người phát ra nhiều yêu cầu cùng một lúc. *[Traffic load balancing](https://sre.google/workbook/managing-load/)* (Cân bằng tải traffic) là cách chúng tôi quyết định máy nào trong số vô vàn máy ở các datacenter sẽ phục vụ một yêu cầu cụ thể. Về lý tưởng, traffic được phân phối xuyên suốt nhiều liên kết mạng, datacenter, và máy theo cách "tối ưu". Nhưng "tối ưu" ở đây có nghĩa là gì? Thực tế không có một câu trả lời đơn lẻ, vì giải pháp tối ưu phụ thuộc mạnh mẽ vào một số yếu tố:
...
Hãy bắt đầu bằng cách xem xét hai kịch bản traffic phổ biến: một yêu cầu tìm kiếm cơ bản và một yêu cầu tải video (upload). Người dùng muốn nhận kết quả truy vấn nhanh chóng, nên biến quan trọng nhất cho yêu cầu tìm kiếm là độ trễ (latency). Mặt khác, người dùng chấp nhận việc upload video mất một khoảng thời gian không thể bỏ qua, nhưng cũng muốn những yêu cầu đó thành công ngay lần đầu, nên biến quan trọng nhất cho video upload là băng thông (throughput). Những nhu cầu khác nhau của hai loại yêu cầu này đóng vai trò trong cách chúng tôi xác định sự phân phối tối ưu cho mỗi yêu cầu ở cấp *toàn cầu*:

-   Yêu cầu tìm kiếm được gửi đến datacenter khả dụng gần nhất — được đo bằng thời gian đi-và-đến (round-trip time, RTT) — vì chúng tôi muốn tối thiểu hóa độ trễ trên yêu cầu.
-   Luồng video upload được định tuyến qua một đường khác — có thể đến một liên kết hiện đang bị khai thác dưới — để tối đa hóa băng thông với chi phí của độ trễ.
...
Tất nhiên, ví dụ này chỉ trình bày một bức tranh đơn giản hóa đáng kể. Trong thực tế, nhiều yếu tố hơn được tính đến trong phân phối [tải tối ưu](https://sre.google/sre-book/load-balancing-datacenter/): một số yêu cầu có thể được định tuyến đến một datacenter hơi xa hơn để giữ cache (bộ đệm) ấm, hoặc traffic tương tác (interactive) không thể được định tuyến đến một vùng hoàn toàn khác để tránh tắc nghẽn mạng. Cân bằng tải, đặc biệt cho các hệ thống lớn, không hề đơn giản hay tĩnh. Tại Google, chúng tôi đã tiếp cận vấn đề bằng cách cân bằng tải ở nhiều cấp, trong đó hai cấp được mô tả trong các phần sau. Để thảo luận cụ thể, chúng tôi sẽ xem xét các yêu cầu HTTP được gửi qua TCP. Cân bằng tải của các dịch vụ không có trạng thái (stateless, như DNS qua UDP) khác đi một chút, nhưng phần lớn các cơ chế được mô tả ở đây vẫn áp dụng được cho các dịch vụ có trạng thái.

## Cân bằng Tải Sử dụng DNS

Trước khi một client (khách hàng) thậm chí có thể gửi một yêu cầu HTTP, nó thường phải tra cứu một địa chỉ IP thông qua DNS. Điều này mở ra cơ hội hoàn hảo để giới thiệu tầng cân bằng tải đầu tiên của chúng tôi: *cân bằng tải DNS*. Giải pháp đơn giản nhất là trả về nhiều bản ghi `A` hoặc `AAAA` trong phản hồi DNS và để client chọn một địa chỉ IP tùy ý. Trong khi đơn giản về mặt khái niệm và dễ cài đặt, giải pháp này đặt ra nhiều thách thức.

Vấn đề đầu tiên là nó cho rất ít khả năng kiểm soát hành vi client: các bản ghi được chọn ngẫu nhiên, và mỗi bản sẽ thu hút một lượng traffic xấp xỉ bằng nhau. Chúng tôi có thể giảm nhẹ vấn đề này không? Về mặt lý thuyết, chúng tôi có thể dùng các bản ghi `SRV` để chỉ định trọng số và thứ tự ưu tiên, nhưng các bản ghi `SRV` chưa được áp dụng cho HTTP.

Một vấn đề tiềm tàng khác bắt nguồn từ thực tế là client thường không thể xác định được địa chỉ gần nhất. Chúng tôi *có thể* giảm nhẹ kịch bản này bằng cách dùng một địa chỉ anycast cho các nameserver (server tên) có thẩm quyền và tận dụng thực tế rằng các truy vấn DNS sẽ chảy đến địa chỉ gần nhất. Trong phản hồi, server có thể trả về các địa chỉ được định tuyến đến datacenter gần nhất. Một cải tiến hơn nữa là xây dựng một bản đồ của tất cả các mạng cùng vị trí vật lý xấp xỉ của chúng, rồi phục vụ các phản hồi DNS dựa trên ánh xạ đó. Tuy nhiên, giải pháp này đến với cái giá của một cài đặt server DNS phức tạp hơn nhiều, cùng việc duy trì một đường ống (pipeline) giữ cho bản đồ vị trí luôn cập nhật.

Tất nhiên, không có giải pháp nào trong số này là tầm thường, do một đặc tính cơ bản của DNS: người dùng cuối hiếm khi nói chuyện trực tiếp với nameserver có thẩm quyền. Thay vào đó, một server DNS đệ quy (recursive) thường nằm ở đâu đó giữa người dùng cuối và nameserver. Server này proxy (chuyển tiếp) các truy vấn giữa người dùng (user) và server, và thường cung cấp một tầng cache (bộ đệm). Người trung gian DNS có ba hệ quả rất quan trọng đối với việc quản lý traffic:
...
Không chỉ khó khăn trong việc tìm địa chỉ IP tối ưu để trả về cho một yêu cầu nhất định của user, mà chính nameserver đó có thể phải phục vụ hàng nghìn hoặc hàng triệu user, trải dài qua các vùng từ một văn phòng đơn lẻ đến cả một châu lục. Ví dụ, một ISP (nhà cung cấp dịch vụ Internet) quốc gia lớn có thể chạy nameserver cho toàn bộ mạng của mình từ một datacenter, nhưng lại có kết nối mạng ở mỗi khu vực đô thị. Khi đó, nameserver của ISP sẽ trả về một phản hồi với địa chỉ IP phù hợp nhất cho datacenter của họ, mặc dù tồn tại các đường mạng tốt hơn cho phần lớn user!
...
-   Chúng tôi ước tính sự phân bố địa lý của các user đằng sau mỗi trình phân giải được theo dõi, để tăng khả năng định hướng những user đó đến vị trí tốt nhất.

Việc ước tính sự phân bố địa lý đặc biệt khó khăn nếu cơ sở user trải dài qua các vùng lớn. Trong những trường hợp như vậy, chúng tôi thực hiện các đánh đổi để chọn vị trí tốt nhất và tối ưu hóa trải nghiệm cho đa số user.

Nhưng "vị trí tốt nhất" thực sự có nghĩa là gì trong ngữ cảnh của cân bằng tải DNS? Câu trả lời hiển nhiên nhất là vị trí gần user nhất. Tuy nhiên (chưa nói đến việc xác định vị trí của user đã khó trong bản thân nó), còn có các tiêu chí bổ sung. Bộ cân bằng tải DNS cần đảm bảo rằng datacenter mà nó chọn có đủ năng lực để phục vụ các yêu cầu từ những user có khả năng cao sẽ nhận được phản hồi của nó. Nó cũng cần biết rằng datacenter được chọn cùng kết nối mạng của nó đang ở trạng thái tốt, vì định hướng các yêu cầu user đến một datacenter đang gặp sự cố điện hoặc mạng là điều không mong muốn. May mắn thay, chúng tôi có thể tích hợp server DNS có thẩm quyền với các hệ thống kiểm soát toàn cầu theo dõi traffic, năng lực, và trạng thái hạ tầng của mình.

Hệ quả thứ ba của người trung gian DNS liên quan đến cache. Giả định rằng nameserver có thẩm quyền không thể flush (xóa) cache của trình phân giải, các bản ghi DNS cần một TTL tương đối thấp. Điều này thực chất đặt một giới hạn dưới cho tốc độ mà các thay đổi DNS có thể lan truyền đến user.<sup>[2](#fn2)</sup> Thật không may, có rất ít điều chúng tôi có thể làm khác ngoài việc ghi nhớ điều này khi đưa ra các quyết định cân bằng tải.

Bất chấp tất cả những vấn đề này, DNS vẫn là cách đơn giản nhất và hiệu quả nhất để cân bằng tải trước khi kết nối của user thậm chí bắt đầu. Mặt khác, cần rõ ràng rằng cân bằng tải chỉ bằng DNS là không đủ. Hãy lưu ý rằng tất cả các phản hồi DNS được phục vụ nên nằm trong giới hạn 512 byte<sup>[3](#fn3)</sup> do RFC 1035 [[Moc87]](https://sre.google/sre-book/bibliography#Moc87) đặt ra. Giới hạn này đặt một giới hạn trên cho số địa chỉ chúng tôi có thể nhồi vào một phản hồi DNS đơn lẻ, và con số đó gần như chắc chắn ít hơn số lượng server của chúng tôi.

Để *thực sự* giải quyết vấn đề cân bằng tải frontend, cấp cân bằng tải DNS ban đầu này nên được theo sau bởi một cấp tận dụng các địa chỉ IP ảo (virtual IP).

## Cân bằng Tải tại Địa chỉ IP Ảo (Load Balancing at the Virtual IP Address)

Các địa chỉ IP ảo (VIPs) không được gán cho bất kỳ giao diện mạng nào cụ thể. Thay vào đó, chúng thường được chia sẻ xuyên suốt nhiều thiết bị. Tuy nhiên, từ quan điểm của user, VIP vẫn là một địa chỉ IP đơn lẻ, bình thường. Về mặt lý thuyết, thực hành này cho phép chúng tôi che giấu các chi tiết cài đặt (như số lượng máy phía sau một VIP nhất định) và tạo thuận lợi cho việc bảo trì, vì chúng tôi có thể lên lịch các nâng cấp hoặc thêm nhiều máy vào bể (pool) mà user không hề hay biết.
...
Giải pháp cân bằng tải VIP hiện tại của chúng tôi [[Eis16]](https://sre.google/sre-book/bibliography#Eis16) sử dụng việc đóng gói packet (packet encapsulation). Một network load balancer đặt packet được chuyển vào một packet IP khác bằng Generic Routing Encapsulation (đóng gói định tuyến tổng quát, GRE) [[Han94]](https://sre.google/sre-book/bibliography#Han94), và dùng địa chỉ của một backend làm đích. Backend nhận packet sẽ tách bỏ tầng IP+GRE bên ngoài và xử lý packet IP bên trong như thể nó được giao trực tiếp đến giao diện mạng của mình. Network load balancer và backend không còn cần tồn tại trong cùng một miền phát sóng; chúng thậm chí có thể ở các châu lục riêng biệt, miễn là tồn tại một đường giữa hai bên.

Việc đóng gói packet là một cơ chế mạnh mẽ, cung cấp sự linh hoạt lớn trong cách các mạng của chúng tôi được thiết kế và tiến hóa. Thật không may, việc đóng gói cũng đi kèm một cái giá: kích thước packet phình to. Việc đóng gói giới thiệu một overhead (chi phí phụ) (chính xác hơn là 24 byte trong trường hợp IPv4+GRE), điều này có thể khiến packet vượt quá kích thước Maximum Transmission Unit (đơn vị truyền tải tối đa, MTU) khả dụng và đòi hỏi phải phân mảnh (fragmentation).
